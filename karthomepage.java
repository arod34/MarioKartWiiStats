...
public class karthomepage implements ActionListener{
	...
	ImageIcon background = new ImageIcon(getClass().getClassLoader().getResource("mkwiibackground.png"));
	...
	// button to change to kart stats
	JButton kartButton = new JButton("Bike Statistics");
	
	boolean kartBool = false;
	...
	private void addButtons() {
		...
		kartButton.setBounds (1500,500,250,50);
		kartButton.setFocusable(false);
		kartButton.addActionListener(this);
		...
		homePage.add(kartButton);

	}

	private int totalLuck() {
		String luck1 = "";String luck2 = ""; int total = 0; int total2 = 0; int count = 0; String file = "kartcoursestats";
		...
		try {
			...
			scan = new Scanner(new InputStreamReader(getClass().getResourceAsStream("kartcoursestats")));
			scan.useDelimiter("[,\n]");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == adamButton) {
			new kartpersonalstats(true);
			homePage.dispose();
		}

		// brian personal stats page
		if (e.getSource() == brianButton) {
			new kartpersonalstats(false);
			homePage.dispose();
		}
		
		if (e.getSource() == kartButton) {
			new HomePage();
			homePage.dispose();
		}
