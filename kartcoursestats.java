...
public class kartcoursestats implements ActionListener{
	...
	public void layout(int courseNum) {
		...
		int adam_best = 0; // int for times i beat brian
		int brian_best = 0; // int for times brian beat me
		...
		try {
			...
			x.useDelimiter("[,\n]");
			y = new Scanner(new InputStreamReader(getClass().getResourceAsStream("courseStatsNew")));
			y.useDelimiter("[,\n]");
			
			
			
			// displaying data from the course stats page
			while (x.hasNext()) {
			...
			// displaying the past five races for the course and how many times either player has preformed better than the other
			while (y.hasNext()) {
				count += 1;
				y.next();
				...
				// adams past 5 and how many times i have bested brian the dirty fucking cheater
				for (int i = 0; i < 12; i++) {
					if (i < 5) {
				...
				if (count == courseNum) {
					...
					adam_five.setBounds(500,800,300,100);
					frame.add(adam_five);
				}
